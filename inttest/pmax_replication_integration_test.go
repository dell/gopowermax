...
var (
	snapID                     string
	sourceVolume, targetVolume *v100.Volume
)
...
func getOrCreateVolumes(client pmax.Pmax, target bool) (*v100.Volume, *v100.Volume, error) {
...
func createSnapshot(sourceVolumeList []v100.VolumeList) (string, error) {
...
func getOrCreateSnapshot(volumeID string, client pmax.Pmax) (string, error) {
	var err error
	if snapID == "" {
		sourceVolumeList := []v100.VolumeList{{Name: volumeID}}
...
func TestCreateSnapshot(t *testing.T) {
	...
	sourceVolumeList := []v100.VolumeList{{Name: srcVolume.VolumeID}}
...
func TestGetVolumeSnapInfo(t *testing.T) {
	...
	sourceVolumeList := []v100.VolumeList{{Name: volume.VolumeID}}
...
func TestSnapshotLinkage(t *testing.T) {
	...
	sourceVolumeList := []v100.VolumeList{{Name: srcVolume.VolumeID}}
	targetVolumeList := []v100.VolumeList{{Name: tgtVolume.VolumeID}}
...
func modifySnapshotLink(sourceVolumeList, targetVolumeList []v100.VolumeList, operation, snapshotName string, t *testing.T) error {
...
func renameSnapshot(symmetrixID, snapshotName, newSnapID string, generation int, sourceVolumeList, targetVolumeList []v100.VolumeList) error {
