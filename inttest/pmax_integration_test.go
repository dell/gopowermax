...
var (
	...
	localVol, remoteVol          *v100.Volume
)
...
func createStorageGroup(symmetrixID, storageGroupID, srp, serviceLevel string, isThick bool) (*v100.StorageGroup, error) {
	if client == nil {
		err := getClient()
		if err != nil {
			return nil, err
...
func TestGetJobs(t *testing.T) {
	...
	jobIDList, err = client.GetJobIDList(context.TODO(), symmetrixID, v100.JobStatusRunning)
	if err != nil {
		t.Error("failed to get Job ID LIst")
		return
	}
	for i, id := range jobIDList {
		...
		fmt.Printf("%s\n", client.JobToString(job))
		if job.Status != v100.JobStatusRunning {
...
func createHost(symmetrixID, hostID string, initiatorKeys []string, hostFlag *v100.HostFlags) (*v100.Host, error) {
	if client == nil {
		err := getClient()
		if err != nil {
			return nil, err
