...
if [ ${SHORT} = "yes" ]; then
    ...
    go test -run "TestExecuteReplicationActionOnSG" -v -short -timeout 90m -coverprofile=c.out -coverpkg github.com/dell/gopowermax/v2 $integrationfiles
else
    go test -run "TestExecuteReplicationActionOnSG" -v -timeout 90m -coverprofile=c.out -coverpkg github.com/dell/gopowermax/v2 $integrationfiles
fi
