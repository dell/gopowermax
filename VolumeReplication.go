...
//Commit storageGroup 
func (c *Client) commitSGMigration(ctx context.Context, sourceSymId, sgName string) (x, error) {
	defer c.TimeSpent("CommitSGMigration", time.Now())

	// check required or not
	if _, err := c.IsAllowedArray(sourceSymId); err != nil {
		return nil, err
	}
	// doubt ----> Structure ??
	commitEnvPayload := &v100.CommitMigration{
		RemoteSymmID: remotesymmID,
	}
	Debug = true
	ifDebugLogPayload(commitEnvPayload)
	URL := c.urlPrefix() + XMigration + SymmetrixX + sourceSymId

	ctx, cancel := c.GetTimeoutContext(ctx)
	defer cancel()

	resp, err := c.api.DoAndGetResponseBody(
		ctx, http.MethodPost, URL, c.getDefaultHeaders(), commitEnvPayload)
	if err = c.checkResponse(resp); err != nil {
		return nil, err
	}
	defer resp.Body.Close()

	// doubt --> Structure to verify ??
	/*decoder := json.NewDecoder(resp.Body)

	MigEnv := new(v100.MigrationEnv)
...
//getorCreateMigrationEnvironment validates existence of or creates migration environment between source and target arrays
func (c *Client) getorCreateMigrationEnvironment(ctx context.Context, sourceSymId, remoteSymId string) (*v100.MigrationEnv, error) {
	...
	createEnvPayload := &v100.CreateMigrationEnv{
		RemoteSymmID: remotesymmID,
	}
	Debug = true
...
//getorCreateMigrationEnvironment validates existence of or creates migration environment between source and target arrays
func (c *Client) removeMigrationEnvironment(ctx context.Context, sourceSymId, remoteSymId string) (error) {
	defer c.TimeSpent("RemoveMigrationEnvironment", time.Now())

	// check required or not
	if _, err := c.IsAllowedArray(sourceSymId); err != nil {
		return nil, err
	
	URL := c.urlPrefix() + XMigration + SymmetrixX + sourceSymId + "/environment/" + remoteSymId
...
func (c *Client) getorCreateSGMigration(ctx context.Context, sourceSymId, remoteSymId , sgName string) (*v100.MigrationEnv, error) {
	defer c.TimeSpent("getorCreateSGMigration", time.Now())
	...
	sgMigPayload := &v100.SGMigration{
		RemoteSymmID: remotesymmID,
	}
	Debug = true
	ifDebugLogPayload(sgMigPayload)
	URL := c.urlPrefix() + XMigration + SymmetrixX + sourceSymId + "/" + sgName
